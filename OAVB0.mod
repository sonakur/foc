...
PROCEDURE Read(VAR R:OIO.BinFile; VAR x: INTEGER);
    VAR byte: CHAR;
  BEGIN 
	OIO.ReadBinChar(R, byte); 
	x := SHORT(ASH(ASH(LONG(ORD(byte)), 24), -24))  (* sign extension *)
  END Read;


PROCEDURE InType(VAR R: OIO.BinFile; VAR type: Type; VAR typdesc: ARRAY OF Type);
    VAR key, tmp: LONGINT;
  ...
  BEGIN Read(R, ref);
    ...
    ELSE Read(R, form);
      IF form = ImpTyp THEN (*re-imported type, module anchor*)
        OIO.ReadBinLInt(R, key); 
	OIO.ReadBinString(R, modname); 
	OIO.ReadBinString(R, typname);
      ...
      ELSE NEW(t); t.form := form;;
		...
		fld.class := class; 
		fld.expo := TRUE; 
		InType(R, fld.type, typdesc); 
		OIO.ReadBinLInt(R, fld.val);            
		fld.next := obj; 
		OIO.ReadBinString(R, fld.name); 
		obj := fld; 
		Read(R, class)
          END ;
	t.dsc := obj
...
(*-------------------------------------------------------------*)
  ...
  BEGIN	
	...
	IF modid = "SYSTEM" THEN
        	mod := ThisModule(modid, modid, key); 
		mod.anc := system
	ELSE 
		MakeFileName(modid1, fname, ".smb"); 
		isopened:=OIO.OpenBinFile(fname, F);
      		IF isopened # FALSE THEN
        	(*AosFS.OpenReader(R, F, 0); R.RawLInt( key ); R.RawString( modname);*)
			OIO.ReadBinLInt(F, key);
			OIO.ReadBinString(F, modname);
        		typdesc[Byte] := byteType; 
			typdesc[Bool] := boolType; 
			typdesc[Char] := charType; 
			typdesc[Int] := intType;        	
			typdesc[Real] := realType; 
			typdesc[Set] := setType; 
			typdesc[String] := strType; 
			typdesc[NoTyp] := noType;
        		mod := ThisModule(modid, modid1, key);
        		Read(F, class);
        		WHILE class # 0 DO
				NEW(new); 
				new.class := class; 
				InType(F, new.type, typdesc);
				IF class = Const THEN
	        			IF new.type.form = Real THEN 
						OIO.ReadBinLInt(F, new.val) 
					ELSE 
						OIO.ReadBinLInt(F, new.val) 
					END
					...
					OIO.ReadBinLInt(F, new.val); 
					new.rdo := TRUE;
          				IF new.type.form = String THEN 
						OIO.ReadBinLInt(F, i); (*len*) 
						new.val := i*10000H + new.val 
	  				END
          			END ;
          			OIO.ReadBinString(F, new.name); 
	  			new.lev := -mod.lev;
          			obj := mod.anc;
...
(*-------------------------------- Export ---------------------------------*)
  ...
  PROCEDURE Write(VAR R: OIO.BinFile; x: INTEGER);
	VAR ch:CHAR;
  	BEGIN 
		ch:=CHR(x);
		OIO.WriteBinChar (R, ch)  (* -128 <= x < 128 *)
  	END Write;

  PROCEDURE OutType(VAR R: OIO.BinFile; t: Type);
    VAR obj, last: Object; tmp_num: SHORTINT;

    PROCEDURE OutPar(VAR R: OIO.BinFile; par: Object; n: INTEGER);
      VAR cl: INTEGER; 	
    BEGIN
      IF n > 0 THEN
    ...
    ELSE
      IF t.typobj # NIL THEN
        ...
        IF t.typobj.lev < 0 THEN (*imported type*);
          Write(R, ImpTyp); 
	  OIO.ReadBinLInt(R, t.typobj.anc.val); (*key*)
          OIO.ReadBinString(R, t.typobj.anc(Module).name1); 
	  OIO.ReadBinString(R, t.typobj.name);
          Write(R, Ref-1)
        END
      ELSE Write(R, 0)
      END ;

      OIO.ReadBinSInt(R, tmp_num);
	t.form:=tmp_num;
      IF t.form = Pointer THEN OutType(R, t.base)
      ELSIF t.form = Array THEN OutType(R, t.base); OIO.ReadBinLInt(R, t.len); OIO.ReadBinLInt(R, t.size)
      ELSIF t.form = Record THEN
        IF t.base # NIL THEN OutType(R, t.base); last := t.base.dsc;
        ELSE OutType(R, noType); last := NIL
        END ;
        OIO.ReadBinInt(R, t.nofpar); OIO.ReadBinLInt(R, t.size); obj := t.dsc;
        WHILE obj # last DO
          IF obj.expo THEN	
            OIO.ReadBinSInt(R, tmp_num);
	    obj.class:=tmp_num; 
	    OutType(R, obj.type);
            OIO.ReadBinLInt(R, obj.val); (*offset*) 
	    OIO.ReadBinString(R, obj.name)
          END ;
          obj := obj.next
        END ;
        Write(R, 0)
  ...
  END OutType;
    ...
    VAR x, sum, oldkey, expno: LONGINT;
      ...
      F: OIO.BinFile; 
      is_opened: BOOLEAN;
      (*R: AosFS.Reader; W: AosFS.Writer;*)
  BEGIN 
	oldkey:=0;
	Ref := ImpTyp + 1; 
	expno := 1; 
	MakeFileName(modid, filename, ".smb");    
	is_opened := OIO.OpenBinFile(filename, F);	    
	IF is_opened # FALSE THEN (*AosFS.OpenReader(R, F, 0); R.RawLInt( oldkey)*) END ;    
	key := oldkey;
	is_opened := OIO.OpenNewBinFile(filename, F); (*AosFS.OpenWriter(W, F, 0);*)
	OIO.WriteBinLInt(F, 0); 
	...
	OutType(F, obj.type);
        IF obj.class = Const THEN
          IF obj.type.form = Proc THEN 
		OIO.WriteBinLInt(F, expno); 
		INC(expno)
          ELSIF obj.type.form = Real THEN OIO.WriteBinLInt(F, obj.val)
          ELSE OIO.WriteBinLInt(F, obj.val)
          END
        ELSIF (obj.class = Var) OR (obj.class = Typ) THEN          
		OIO.WriteBinLInt(F, expno); 
		INC(expno);          
		IF obj.type.form = String THEN  (*clear length for OSAG.Close*)			
			OIO.WriteBinLInt(F, obj.val DIV 10000H); 
			obj.val := obj.val MOD 10000H          
		END
        END ;
      ...
      obj := obj.next
    END ;
    REPEAT Write(F, 0) UNTIL OIO.BinFileLength(F) MOD 4 = 0;
    (*W.Update();  F.Update();*)
	OIO.BinFileUpdate(F);    
	(*AosFS.OpenReader(R, F, 0);*) 
	sum := 0;
    WHILE OIO.Available(F) # 0 DO 
	OIO.ReadBinLInt(F, x); 
	sum := sum + x 
	END ;
    IF newSF THEN
	...
	OIO.WriteBinLInt(F, sum);
        (* fof removed Files.Register(F)  *) 
	OIO.BinFileUpdate(F);  
	OIO.Register(F) (*insert checksum at the beginning*)
      ELSE newSF := FALSE
      END
